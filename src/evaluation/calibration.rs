//! Outcome calibration: Brier score, hit-time statistics, agreement-bucketed reliability, and
...
//! not how well a signal's *agreement* predicted its outcome.
...
/// One agreement bucket's calibration: how predicted agreement compared to the actual win rate
...
pub struct CalibrationReport {
    /// Mean squared error between predicted agreement (as a 0.0..=1.0 win probability) and the
...
/// Computes a [`CalibrationReport`] from `records`, using `agreement` (expected `0.0..=1.0`) as
/// each record's predicted win probability. `num_buckets` controls the reliability-diagram
/// resolution (records are sorted by agreement and split into roughly equal-sized buckets).
...
mod tests {
    ...
    fn test_buckets_reveal_miscalibration() {
        // High agreement but only ever loses: the bucket's observed win rate must diverge
