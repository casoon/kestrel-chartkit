...
pub struct SignalEvaluationRecord {
    ...
    pub score: f64,
    /// 0..1 — wie einig sich die Eingangsgrößen waren, als das Signal
    /// entstand. **Keine behauptete Wahrscheinlichkeit.**
    ///
    /// Hieß bis 2026-09-08 `confidence`. Genau das war der Denkfehler: die
    /// Kalibrierung prüft, ob ein Wert Ausgänge vorhersagt — sie darf ihn
    /// nicht schon im Namen als Vorhersage führen. Die Frage lautet „sagt
    /// Einigkeit etwas über den Ausgang?", und die Antwort steht in
    /// `calibration`, nicht im Feldnamen.
    pub agreement: f64,
    pub entry_price: f64,
    pub exit_price: f64,
