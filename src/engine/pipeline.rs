...
/// Optional empirical measurements to refine heuristic market state values.
#[derive(Debug, Clone, Default)]
pub struct MeasuredMarketInputs<'a> {
    /// Measured empirical balance probability (e.g. from Markov/regime persistence).
    pub empirical_balance_prob: Option<f64>,
    /// Opening range width (`orb_high - orb_low`) of the current session.
    pub current_orb_range: Option<f64>,
    /// Historical opening range widths from previous sessions for percentile calculation.
    pub session_orb_history: &'a [f64],
}

/// Computes `MarketStateOutput` and `ExpectedPlaybook` using empirical balance persistence and session ORB percentile.
///
/// If `measured.empirical_balance_prob` is provided, it replaces the static 0.8/0.2 regime heuristic.
/// If `measured.session_orb_history` contains past opening ranges, `opening_range_percentile` is computed via percentile rank.
pub fn build_market_state_measured(
    bars: &[Bar],
    adx_val: f64,
    atr_val: f64,
    trend_stability: f64,
    opening_type: OpeningType,
    measured: &MeasuredMarketInputs<'_>,
) -> (MarketStateOutput, ExpectedPlaybook) {
    let (mut state_out, playbook) =
        build_market_state_and_playbook(bars, adx_val, atr_val, trend_stability, opening_type);

    if let Some(p) = measured.empirical_balance_prob {
        state_out.balance_probability = p.clamp(0.0, 1.0);
    }

    if let Some(curr_orb) = measured.current_orb_range {
        if !measured.session_orb_history.is_empty() {
            let rank = crate::stats::percent_rank(measured.session_orb_history, curr_orb);
            state_out.opening_range_percentile = (rank / 100.0).clamp(0.0, 1.0);
        }
    }
