...
//!
//! Covers the triangle family (symmetrical, ascending, descending) and its diverging counterpart
//! the broadening formation, Rising/Falling Wedge, the 1-2-3 Reversal, the Wolfe Wave, the classic
...
pub enum ChartPatternKind {
    /// Symmetrical triangle: both boundaries slope towards each other.
    Triangle,
    /// Flat resistance, rising support.
    AscendingTriangle,
    /// Flat support, falling resistance.
    DescendingTriangle,
    /// Diverging boundaries — the range widens instead of narrowing.
    Broadening,
    RisingWedge,
    FallingWedge,
    ReversalOneTwoThree,
...
pub struct ChartPatternCandidate {
    ...
    /// 0..1 — wie gut die gefundene Form dem Muster entspricht.
    ///
    /// Hier heißt `confidence` zu Recht: es ist die Sicherheit des
    /// *Erkenners* über eine Form, keine Aussage über den Ausgang eines
    /// Trades. Nicht zu verwechseln mit dem, was bis 2026-09-08 in
    /// `scoring::Agreement` und `CompositeSignal` so hieß und dort
    /// Einigkeit maß — das wurde umbenannt, dieses Feld bewusst nicht.
    pub confidence: f64,
}
...
impl ChartPatternCandidate {
    ...
    pub fn update_state(&mut self, bar: &Bar) -> PatternState {
        ...
        self.state = match self.kind {
            ...
            | ChartPatternKind::AscendingTriangle
            | ChartPatternKind::DescendingTriangle
            | ChartPatternKind::Broadening
...
pub struct ChartPatternDetector {
    ...
    /// How much a boundary may rise or fall across the window and still count as horizontal,
    /// as a percentage of the window's opening gap between the two boundaries.
    ///
    /// Its own threshold rather than a reuse of `tolerance_pct`: that one answers "are these two
    /// prices the same level", this one answers "is this line flat". Both are decisions, but they
    /// are not the same decision, and a caller that widens level matching to find more double tops
    /// should not thereby start seeing ascending triangles everywhere.
    pub flat_pct: f64,
}
...
impl ChartPatternDetector {
    ...
    /// Sets the flatness threshold for the ascending/descending triangle boundary.
    pub fn with_flat_pct(mut self, flat_pct: f64) -> Self {
        self.flat_pct = flat_pct.max(0.0);
        self
    }
    ...
    ///
    /// # Why the flat cases are their own kinds
    ///
    /// A symmetrical triangle, an ascending triangle and a descending triangle are all "two
    /// converging boundaries", but they are not the same condition: one boundary being horizontal
    /// is an *additional* requirement, and the literature reads the three differently. Folding
    /// them into one kind would report the narrower case under the wider name — the caller could
    /// not tell whether a horizontal boundary was found or merely not ruled out.
    ///
    /// The broadening formation is the same window scan with the convergence test reversed. It
    /// shares the scan because it shares the geometry: four alternating nodes, two lines. Only the
    /// sign of the gap change differs.
    fn scan_triangles_and_wedges(&self, nodes: &[ZigZagNode]) -> Vec<ChartPatternCandidate> {
        ...
        for window in nodes.windows(4) {
            ...
            if gap_start <= 0.0 || gap_end <= 0.0 {
                continue; // the lines cross — not a channel-shaped window at all
            }

            // Flatness as the rise *across the window*, relative to the gap the window opens
            // with — both prices, so the ratio is dimensionless. Comparing `slope()` to a price
            // gap instead, as this did before, compares a price-per-millisecond to a price: on
            // real millisecond timestamps every line comes out flat.
            let flat_tol = self.flat_pct / 100.0;
            let rise = |line: &TrendLine| (line.value_at(end_ts) - line.value_at(start_ts)).abs();
            let upper_flat = rise(&upper) / gap_start.max(1e-9) < flat_tol;
            let lower_flat = rise(&lower) / gap_start.max(1e-9) < flat_tol;

            // The diverging case first: a broadening formation is the one shape here that widens.
            // It needs both boundaries to actually move apart — a single sloping line against a
            // flat one widens too, but that is a right-angled variant and reads as a triangle
            // turned around, so it is held to the same "both sides move" bar as the wedges.
            if gap_end > gap_start {
                if upper_flat || lower_flat || upper.slope() <= 0.0 || lower.slope() >= 0.0 {
                    continue;
                }
                let divergence = 1.0 - (gap_start / gap_end);
                out.push(ChartPatternCandidate {
                    kind: ChartPatternKind::Broadening,
                    nodes: window.to_vec(),
                    upper_line: Some(upper),
                    lower_line: Some(lower),
                    state: PatternState::Forming,
                    confidence: divergence.clamp(0.0, 1.0),
                });
                continue;
            }

            if gap_end >= gap_start {
                continue; // parallel: a channel, and that is not this detector's business
            }

            let convergence = 1.0 - (gap_end / gap_start);

            // Order matters: the flat cases are the narrower conditions and are tested first, or
            // a flat-topped triangle would be reported as symmetrical.
            let kind = if upper.slope() > 0.0 && lower.slope() > 0.0 {
                ChartPatternKind::RisingWedge
            } else if upper.slope() < 0.0 && lower.slope() < 0.0 {
                ChartPatternKind::FallingWedge
            } else if upper_flat && lower.slope() > 0.0 {
                ChartPatternKind::AscendingTriangle
            } else if lower_flat && upper.slope() < 0.0 {
                ChartPatternKind::DescendingTriangle
            } else if upper.slope() <= 0.0 && lower.slope() >= 0.0 {
                ChartPatternKind::Triangle
            } else {
...
mod tests {
    ...
    #[test]
    fn test_ascending_triangle_needs_a_flat_top() {
        // Upper boundary 110 → 110 (flat), lower 90 → 100 (rising): the gap narrows from one
        // side only. That is the defining asymmetry.
        let nodes = vec![
            node(0, 110.0, true),
            node(10, 90.0, false),
            node(20, 110.0, true),
        ...
        let candidates = ChartPatternDetector::new(2.0).scan(&nodes);
        assert!(candidates
            .iter()
            .any(|c| c.kind == ChartPatternKind::AscendingTriangle));
    }

    #[test]
    fn test_descending_triangle_mirrors() {
        let nodes = vec![
            node(0, 90.0, false),
            node(10, 110.0, true),
            node(20, 90.0, false),
        ...
        assert!(candidates
            .iter()
            .any(|c| c.kind == ChartPatternKind::DescendingTriangle));
    }

    #[test]
    fn test_a_sloping_top_is_symmetrical_not_ascending() {
        // Same shape as the ascending case, but the top drops 110 → 104. Nothing here is
        // horizontal, so the narrower name must not be used.
        let nodes = vec![
            node(0, 110.0, true),
            node(10, 90.0, false),
            node(20, 104.0, true),
    ...
    #[test]
    fn test_detects_broadening_formation() {
        // Highs rise, lows fall: the range widens. The converging detectors must stay silent.
        let nodes = vec![
            node(0, 105.0, true),
            node(10, 95.0, false),
            node(20, 115.0, true),
        ...
        assert!(candidates
            .iter()
            .any(|c| c.kind == ChartPatternKind::Broadening));
    ...
    #[test]
    fn test_broadening_confirms_on_breakout() {
        let nodes = vec![
            node(0, 105.0, true),
            node(10, 95.0, false),
            node(20, 115.0, true),
        ...
        let mut candidate = ChartPatternDetector::new(2.0)
            .scan(&nodes)
            .into_iter()
            .find(|c| c.kind == ChartPatternKind::Broadening)
            .expect("broadening candidate");
        assert_eq!(candidate.state, PatternState::Forming);

        let inside = Bar::new(40, 100.0, 100.0, 100.0, 100.0, 0.0);
        assert_eq!(candidate.update_state(&inside), PatternState::Forming);

        // The threshold moves away from price as the window is extrapolated — that is inherent
        // to a diverging boundary, not an artefact: at ts 50 the upper line already projects 130.
        let above = Bar::new(50, 145.0, 145.0, 145.0, 145.0, 0.0);
        assert_eq!(candidate.update_state(&above), PatternState::Confirmed);
    }
