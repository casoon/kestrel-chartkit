...
pub fn catalog() -> Vec<IndicatorCatalogEntry> {
    vec![
        ...
        IndicatorCatalogEntry {
            ...
            description: "Chandelier Exit ratchet extended with adaptive multiplier, body-filtered weak flips, and bull/bear trap detection (use ChandelierFlipRadarEngine::new directly for use_close_extremes=false or simple_adaptive=true; this f64-only entry uses the documented defaults for both)",
            default_params: [
        ...
        IndicatorCatalogEntry {
            name: "cvd",
            // Der Name verspricht Kauf- minus Verkaufsvolumen, also die
            // Aggressorseite. Diese Fassung schaetzt sie aus der Bar-Form
            // (`(close - low) / (high - low)`) und sieht keinen einzigen
            // Abschluss an — deshalb `needs_trade_direction: false`, was
            // korrekt ist, aber ohne diesen Zusatz irrefuehrend waere.
            description: "Cumulative Volume Delta (Aufteilung aus der Bar-Form geschätzt, \
                          nicht aus klassifizierten Abschlüssen)",
            default_params: [].into(),
        },
        IndicatorCatalogEntry {
            name: "klinger",
...
pub fn build_checked(
    ...
    match name.to_lowercase().as_str() {
        ...
        "chandelier_flip_radar" | "chfr" => {
            ...
            // `use_close_extremes`/`simple_adaptive` are fixed to their documented defaults
            // here (true/false) —
        ...
        "candle_story" | "pinbar" => {
            ...
            Ok(Box::new(CandleStoryEngine::with_config(
                CandleStoryConfig {
                    ...
                    belt_hold_body_min: get_f64_p(
                        params,
                        "belt_hold_body_min",
                        d.belt_hold_body_min,
                        0.3,
                        1.0,
                    )?,
                    belt_hold_open_wick_max: get_f64_p(
                        params,
                        "belt_hold_open_wick_max",
                        d.belt_hold_open_wick_max,
                        0.0,
                        0.3,
                    )?,
