...
impl ExtendedVolumeProfileEngine {
    ...
    fn compute(&mut self) -> Option<IndicatorOutput> {
        ...
        // The lookback window bounds every artifact computed from it.
        let window = match (self.records.front(), self.records.back()) {
        ...
        let mut delta_artifact = ProfileArtifact {
        ...
        if let Some((from, to)) = window {
            ...
            delta_artifact = delta_artifact.spanning(from, to);
        }
