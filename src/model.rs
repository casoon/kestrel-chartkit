...
///
/// Ties together instrument symbol, timeframe, session type, contract continuity, and price adjustment.
/// Can be attached to stored artifacts or downstream results to ensure that price-level findings
/// (e.g. Elliott Waves, pivots, S/R zones) are never evaluated against mismatched data feeds.
...
impl SeriesIdentity {
    pub fn new(symbol: impl Into<String>, timeframe: impl Into<String>) -> Self {
        Self {
            ...
            continuity: ContinuityKind::Spot,
    ...
    pub fn with_continuity(mut self, continuity: ContinuityKind) -> Self {
        self.continuity = continuity;
        self
    }

    pub fn with_price_adjustment(mut self, adj: PriceAdjustment) -> Self {
        self.price_adjustment = adj;
        self
    }
    ...
    pub fn with_contract_code(mut self, code: impl Into<String>) -> Self {
        self.contract_code = Some(code.into());
        self
    }

    /// Returns true if this series is compatible with another series in terms of symbol,
    /// session boundaries, contract continuity, and price adjustment.
    pub fn is_compatible(&self, other: &Self) -> bool {
        self.symbol == other.symbol
            && self.session == other.session
            && self.continuity == other.continuity
            && self.price_adjustment == other.price_adjustment
            && self.contract_code == other.contract_code
    }
}
