//! Reference-parity fixture harness: a standardized way to compare a Rust indicator's output
//! series against confirmed external reference values, with timestamp alignment, automatic warmup
//! handling,
...
/// missing/skip row (a bar with no confirmed reference value, e.g. inside its own warmup).
...
/// validating a port's higher-timeframe output only at points where the confirmed reference HTF value is
