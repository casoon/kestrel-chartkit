...
pub struct ResamplerOutput {
    ...
    /// repaint risk (a bar that is still forming): its OHLC will keep changing
...
/// (repainting) higher-timeframe value through it — the equivalent of a lookahead-free
/// higher-timeframe request combined with confirmed-only consumption.
#[derive(Debug, Clone)]
