...
pub mod scene;
pub use scene::{
    scene_from_artifacts, Axis, AxisKind, LineStyle, Pane, Scene, SceneObject, SceneObjectKind,
};
...
/// The color contract for scene and chart objects.
///
/// A color is a CSS-compatible string in one of these forms:
///
/// - `#rgb`, `#rrggbb`, `#rrggbbaa` — hex literals, the last one carrying alpha
/// - `var(--name)` — a CSS custom property, so a host can theme a rendered chart
///
/// Anything else is replaced by a neutral fallback rather than dropped: an object in
/// the wrong color still carries its information, a missing one does not.
///
/// Renderers other than the built-in SVG one should apply this same function so that
/// a scene looks the same wherever it is drawn.
pub fn sanitize_color(c: &str) -> String {
    let trimmed = c.trim();
