...
def accrued_interest(face, coupon, dates, settlement):
    """Coupon accrued from the start of the period containing `settlement` up to it, Actual/365
    Fixed. Zero on a coupon date, where a new period starts."""
    for start, end in _periods(dates):
        if start <= settlement < end:
            return face * coupon * act365(start, settlement)
    return 0.0


def cashflows(face, coupon, dates, settlement, payment_dates=None):
    ...
    for index, (start, end) in enumerate(_periods(dates)):
        ...
        if paid <= settlement:
            continue
        amount = face * coupon * act365(start, end)
        if index == last:
            amount += face
...
def price_at_yield(face, coupon, frequency, dates, settlement, ytm):
    """Dirty and clean price, accrued interest and durations at one yield.

    Every outstanding cashflow is discounted with (1 + ytm/f)^(-f t), t = Actual/365 Fixed from
    settlement to payment. Macaulay duration is the present-value weighted mean of those t;
    modified duration, -dP/dy / P, is Macaulay / (1 + ytm/f) for this compounding.
    """
    base = 1.0 + ytm / frequency
    dirty = 0.0
    weighted_time = 0.0
    for paid, amount in cashflows(face, coupon, dates, settlement):
        t = act365(settlement, paid)
        present = amount * base ** (-frequency * t)
        dirty += present
        weighted_time += t * present
    accrued = accrued_interest(face, coupon, dates, settlement)
